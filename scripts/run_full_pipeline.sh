...
python methods/encoders/tsp/step1_extract_mvit_gvf.py 2>&1 | tee "$LOG/1_gvf.log"

say "STAGE 2/5: train TSP (dual head: action touch/untouch + GVF-fed FG/BG, 8 epochs)"
bash methods/encoders/tsp/train_tsp_on_hoi4d.sh 2>&1 | tee "$LOG/2_train.log"

say "STAGE 3/5: select best epoch by Foreground-F1"
python methods/encoders/tsp/step3_select_best_f1.py 2>&1 | tee "$LOG/3_selectf1.log"
...
python methods/encoders/tsp/step4_extract_features.py --ckpt "$BEST" 2>&1 | tee "$LOG/4_extract.log"

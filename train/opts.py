...
def parse_args():
    ...
    parser.add_argument('--root-dir', default=None,
                        help='Path to root directory containing the videos files '
                             '(not needed if --frames-dir is given)')
    parser.add_argument('--frames-dir', default=None,
                        help='If set, read clips from pre-extracted JPG frame folders at '
                             '<frames-dir>/<video_name>/{:06d}.jpg instead of decoding videos. '
                             'Avoids ffmpeg/mp4. --root-dir/--*-subdir are ignored for data loading.')
    ...
    assert args.root_dir or args.frames_dir, \
        'Either --root-dir (decode videos) or --frames-dir (read JPG frames) must be given.'

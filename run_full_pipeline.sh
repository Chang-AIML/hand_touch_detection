...
python scripts/step1_extract_mvit_gvf.py 2>&1 | tee "$LOG/1_gvf.log"

say "STAGE 2/5: train TSP (dual head: action touch/untouch + GVF-fed FG/BG, 8 epochs)"
bash train/train_tsp_on_hoi4d.sh 2>&1 | tee "$LOG/2_train.log"

say "STAGE 3/5: select best epoch by Foreground-F1"
python scripts/step3_select_best_f1.py 2>&1 | tee "$LOG/3_selectf1.log"
...
python scripts/step4_extract_features.py --ckpt "$BEST" 2>&1 | tee "$LOG/4_extract.log"

say "STAGE 5/5: downstream MS-TCN then ASFormer on TSP features"
python downstream/train_head.py -m mstcn    2>&1 | tee "$LOG/5_mstcn.log"
python downstream/train_head.py -m asformer 2>&1 | tee "$LOG/6_asformer.log"

...
from dpc.vjepa import OnlineVJEPA                                     # noqa: E402
...
def build_parser():
    ...
    # --- InfoNCE key-separation (opt-in; 0 = off = the verified generation-only path) ---
    ap.add_argument("--infonce_lambda", type=float, default=0.0, help="weight of the InfoNCE key-separation loss on Q-Former event-frame tokens; 0=off. Trains Q-Former+2 proj heads, freezes out.1/LLM/V-JEPA")
    ap.add_argument("--infonce_temp", type=float, default=0.07)
    ap.add_argument("--infonce_bank", type=int, default=512, help="MoCo bank size for cross-batch hard (other-type) negatives")
    ap.add_argument("--infonce_tol", type=int, default=2, help="frames around a GT event counted as positives")
    ap.add_argument("--infonce_joint", type=int, default=0, help="FALLBACK (BLIP-2-style prevent-collapse): tap InfoNCE on POST-out.1 frame tokens (d_llm) and train the FULL connector (out.1 unfrozen) so the separation gradient reaches the readout collapse site; warm-start from an early, less-collapsed ckpt + joint L_gen+InfoNCE. 0=default (tap z_t pre-out.1, freeze out.1)")
    # deprecated flags — the features were ablated out; kept parseable for YAML/script back-compat,
    # asserted off in main(). Do NOT hard-remove: the live dpc-*.yaml + run_*.sh still pass them.
...
def main():
    ...
    for _f in ("stain", "use_text", "gate_lang", "lora"):
        if getattr(a, _f):
            raise SystemExit(f"--{_f} was removed (that feature was ablated out); only 0 is supported")
    if a.stride is None:
    ...
    fc = FrameCompress(768, a.n_q, W.d_llm, n_heads=4).to(dev, torch.float32)
    ...
    trainable = list(fc.parameters())          # the connector is the only trainable module
    nce = None
    if a.infonce_lambda > 0:                    # InfoNCE key-separation
        from dpc.infonce import InfoNCEHeads, KeyBank, infonce as _nce_loss, collapse_stats
        if a.infonce_joint:                      # FALLBACK: tap POST-out.1 frame tokens + train the FULL connector
            heads = InfoNCEHeads(W.d_llm, W.d_llm).to(dev, torch.float32)    # frame dim = d_llm (post-readout token)
            trainable = list(fc.parameters()) + list(heads.parameters())     # out.1 UNFROZEN -> grad reaches collapse site
            tap = f"post-out.1 frame tokens; FULL connector trainable (prevent-collapse from {os.path.basename(a.init_fc or '?')})"
        else:                                    # DEFAULT: tap z_t (pre-out.1), freeze readout, fix the extraction
            for n, p in fc.named_parameters():
                if n.startswith("out."):
                    p.requires_grad_(False)      # out.0/out.1 frozen (the collapse is downstream in the readout)
            heads = InfoNCEHeads(a.n_q * 768, W.d_llm).to(dev, torch.float32)
            trainable = [p for n, p in fc.named_parameters() if not n.startswith("out.")] + list(heads.parameters())
            tap = "z_t (pre-out.1); out.0/out.1 frozen"
        nce = {"heads": heads, "bank": KeyBank(a.infonce_bank), "er": [], "stats": collapse_stats,
               "loss": _nce_loss, "joint": bool(a.infonce_joint)}
        log0(f"[infonce] ON lambda={a.infonce_lambda} temp={a.infonce_temp} bank={a.infonce_bank} | tap={tap}; "
             f"trainable={sum(p.numel() for p in trainable)/1e6:.2f}M")
    loz = Localizer(W, fc, use_anchor=bool(a.use_anchor), anchor_stride=a.anchor_stride,
                    anchor_max_side=a.anchor_max_side, fps=15, max_frames=a.window_frames,
    ...
    # ---------------- optim + wandb (rank 0) ----------------
    opt = torch.optim.AdamW([{"params": trainable, "lr": a.lr}], weight_decay=0.01)
    ...
    if os.path.exists(resume_path):
        ...
        if nce is not None and "heads" in rk:               # restore InfoNCE proj heads (kept outside fc)
            nce["heads"].load_state_dict(rk["heads"])
    ...
    def save_resume(step):
        ...
        if nce is not None:                                 # InfoNCE proj heads live outside fc; save them
            rk["heads"] = nce["heads"].state_dict()         # so resume restores weights, not just their Adam moments
    ...
    def _infonce_step(batch):
        """Query→event-frame multi-positive InfoNCE at the Q-Former output. Returns a scalar (0 if no
        positives). HARD negatives = cross-batch OTHER-type event tokens (the bank) + within-window bg."""
        H = nce["heads"]; bank = nce["bank"]; losses = []
        for s in batch:
            N = int(s["num_frames"]); ev = s.get("event_frames") or []
            pos_idx = sorted({f for e in ev for f in range(int(e) - a.infonce_tol, int(e) + a.infonce_tol + 1) if 0 <= f < N})
            if not pos_idx:
                continue                                                  # negative sample -> no positives
            z = (fc(s["grid"], None, N) if nce["joint"]                   # post-out.1 (N,d_llm): tap where it collapses
                 else fc.qformer_tokens(s["grid"], N))                    # pre-out.1 (N,n_q*768): fix the extraction
            ef = H.frame(z[pos_idx])                                      # (P,256) positives
            q_ids = W.tokenizer(" " + s["question"], add_special_tokens=False).input_ids
            qv = H.query(W.embed_tokens(torch.tensor(q_ids, device=dev)).mean(0))   # pooled query -> (256,)
            neg_pool = [f for f in range(N) if f not in set(pos_idx)]
            if len(neg_pool) > 96:
                neg_pool = [neg_pool[i] for i in torch.randperm(len(neg_pool))[:96].tolist()]
            bg = H.frame(z[neg_pool]) if neg_pool else ef[:0]             # within-window background negatives
            hard = bank.negatives(s["type"])                             # cross-batch OTHER-type (detached)
            neg = torch.cat([bg, hard], 0) if hard is not None else bg
            if neg.shape[0] == 0:
                continue
            losses.append(nce["loss"](qv, ef, neg, a.infonce_temp))
            bank.add(ef.detach(), s["type"])                             # enqueue for future hard negatives
            nce["er"].append(z[pos_idx].detach().float().cpu())          # raw event tokens (tapped rep) for eff-rank
            nce["er"][:] = nce["er"][-64:]
        return torch.stack(losses).mean() if losses else torch.zeros((), device=dev)

    for ep in range(a.epochs):
        fc.train()
        if getattr(train_ds, "temperature_epoch_order", None) is not None:
            order = train_ds.temperature_epoch_order(ep)   # temp-weighted, WITHOUT replacement; identical on all ranks
        else:
        ...
        for wss in batch_iter:
            ...
            gen = loz.loss_batch(batch)
            if nce is not None:                              # + InfoNCE key-separation on Q-Former tokens
                nl = _infonce_step(batch); nce["last_nl"] = float(nl.detach())
                loss = (gen + a.infonce_lambda * nl) / a.grad_accum
            else:
                loss = gen / a.grad_accum                    # scale for accumulation
            loss.backward()
            ...
            if ddp:                                       # all-reduce the small trainable grads
                for p in trainable:                       # zero-fill missing grads so EVERY rank reduces the
                    if p.grad is None:                    # SAME param set (a rank with an all-negative micro-batch
                        p.grad = torch.zeros_like(p)      # gets no InfoNCE grad on the heads -> would desync NCCL)
            ...
            mpc, er = None, None                          # collapse metrics: rank-0 only, every 5 steps (SVD is local)
            if nce is not None and is_main and gstep % 5 == 0 and nce["er"]:
                mpc, er = nce["stats"](torch.cat(nce["er"], 0))
            if is_main and wb is not None:
                d = {"train/loss": cur, "train/loss_avg": rl / micro, "train/lr": sched.get_last_lr()[0],
                     "train/grad_norm": gnorm, "train/epoch": ep}
                if nce is not None:
                    d["train/loss_nce"] = nce.get("last_nl", 0.0)
                    if er is not None:
                        d["train/event_eff_rank"] = er; d["train/event_mean_cos"] = mpc
                wb.log(d, step=gstep)
            if gstep % 5 == 0:
                extra = f" | nce {nce['last_nl']:.3f} eff_rank {er:.2f} mean_cos {mpc:.3f}" if (nce is not None and er is not None) else ""
                log0(f"  ep{ep} step{gstep} loss {cur:.4f} (avg {rl/micro:.4f}) gnorm {gnorm:.2f}{extra} | "

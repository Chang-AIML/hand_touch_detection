"""Evaluate a Stage-1 idx-gen checkpoint (query-compression connector [+LoRA]) on the
TEST split. Confirms the val findings (M1 67.6, SFT-LoRA 72.5) hold on test."""
...
def main():
    ...
    ap.add_argument("--ckpt", required=True); ap.add_argument("--split", default="test")
    ...
    if "lora" in ck:                                             # SFT-LoRA checkpoint
    ...
    vids = [v for v in json.load(open(os.path.join(LAB, f"{a.split}.json")))
    ...
    with torch.no_grad():
        for b0 in range(0, len(items), a.batch_size):
            ch = items[b0:b0 + a.batch_size]
    ...
    print(f"\n=== {os.path.basename(os.path.dirname(a.ckpt))} on {a.split.upper()} "
          f"({len(vids)} vids) ===\n  mAP@0/1/2 = {[round(x, 2) for x in m]}  | ndet={ndet} ngt={ngt}", flush=True)

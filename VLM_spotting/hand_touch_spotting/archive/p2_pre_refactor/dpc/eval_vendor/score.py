"""Vendored from hand_touch_detection/common/score.py (verbatim logic).

Frame-tolerance spotting mAP, identical to the ASTRM / TSP / V-JEPA baseline
tables so numbers stay directly comparable.

INPUT FORMAT
------------
truth : list of per-video dicts
    {"video": <video_id>, "events": [{"label": <cls>, "frame": <int>}, ...], ...}
pred  : list of per-video dicts (SAME video set as truth)
    {"video": <video_id>, "events": [{"label": <cls>, "frame": <int>, "score": <float>}, ...], ...}
    NB: predicted events carry a "score" (confidence); truth events do not.

compute_mAPs(truth, pred, tolerances=[0,1,2,4]) -> (mAPs, tolerances)
    mAPs : list[float] in [0, 1] (fraction, NOT %), one per tolerance,
           macro-averaged over labels. tolerances : the echoed tolerance list.
    Also prints a per-class AP table (via tabulate) to stdout.

The only third-party dep is numpy; `tabulate` is used purely for the pretty
table print and is imported optionally (graceful fallback if unavailable).
"""
import sys
...
try:                                              # pretty table print only
    from tabulate import tabulate
except Exception:                                 # pragma: no cover - fallback
    def tabulate(rows, headers=(), floatfmt=""):
        def fmt(x):
            if isinstance(x, float):
                return format(x, floatfmt) if floatfmt else str(x)
            return str(x)
        lines = []
        if headers:

...
def main(args):
    ...
    if args.frames_dir:
        # read pre-extracted JPG frames directly (no ffmpeg/mp4)
        DatasetClass = FrameUntrimmedVideoDataset
    ...
    # Infer GVF dimensionality from the h5 (may be != clip feature size, e.g. MViT-B GVF = 768)
    gvf_size = None
    if args.global_video_features is not None:
        import h5py
        with h5py.File(args.global_video_features, 'r') as _gf:
            _k = next(iter(_gf.keys()))
            gvf_size = int(_gf[_k].shape[-1])
        print(f'GVF size inferred from {args.global_video_features}: {gvf_size}')

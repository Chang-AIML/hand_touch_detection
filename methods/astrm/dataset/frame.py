...
def _get_deferred_rgb_transform():
    img_transforms = [
        # Paper ASTRM augmentation: brightness/contrast/saturation in
        # [0.7, 1.2] with p=0.5, plus Gaussian blur(5) with p=0.25.
        ...
        if not is_eval:
            ...
            if not defer_transform:
                img_transforms.extend([
                    # Paper ASTRM augmentation: brightness/contrast/saturation
                    # in [0.7, 1.2] with p=0.5, plus Gaussian blur(5) p=0.25.
                    transforms.RandomApply(
                        nn.ModuleList([
                            transforms.ColorJitter(saturation=(0.7, 1.2))
                        ]), p=0.5),
...
class ActionSpotDataset(Dataset):
    ...
    def __init__(
            ...
            mixup=False,
            mixup_alpha=0.2,            # Beta(alpha, alpha) mix factor
                                        # (paper ASTRM uses 0.1)

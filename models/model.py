...
class Model(nn.Module):
    ...
    def __init__(self, backbone, num_classes, num_heads=1, concat_gvf=False, gvf_size=None, progress=True, **kwargs):
        '''
        Args:
            ...
            gvf_size (int): Dimensionality of the GVF vector. Defaults to the clip feature_size
                (original TSP, where GVF == r2plus1d features). Set this when the GVF comes from a
                different encoder (e.g. MViT-B = 768, per E2E-Spot paper B.3) so the concat FC fits.
            progress (bool): If True, displays a progress bar of the download to stderr
            **kwargs: keyword arguments to pass to backbone architecture constructor
        '''
        super().__init__()
        print(f'<Model>: backbone {backbone} num_classes {num_classes} num_heads {num_heads} gvf_size {gvf_size} kwargs {kwargs}')
        ...
        # GVF dimensionality (may differ from clip feature_size, e.g. MViT-B GVF = 768)
        self.gvf_size = gvf_size if gvf_size is not None else self.feature_size
        gvf_concat_size = self.feature_size + self.gvf_size

        if self.num_heads == 1:
            # single head: optionally concat GVF (e.g. a GVF-fed temporal-region head)
            self.fc = Model._build_fc(gvf_concat_size if self.concat_gvf else self.feature_size, num_classes[0])
        else:
            self.fc1 = Model._build_fc(self.feature_size, num_classes[0])
            self.fc2 = Model._build_fc(gvf_concat_size if self.concat_gvf else self.feature_size, num_classes[1])
    ...
    def forward(self, x, gvf=None, return_features=False):
        ...
        if self.num_heads == 1:
            if self.concat_gvf:
                ...
                logits = [self.fc(torch.cat([features, gvf], dim=-1))]
            else:
